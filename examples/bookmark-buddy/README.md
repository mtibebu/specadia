...
1. A short product [intent](intent.md) becomes a reviewed
   [requirements document](requirements.md) and [design document](design.md).
   Those artifacts may be authored by a person, produced with READ-MAS, or
   generated by Specadia with agents enabled. The checked-in files are already
   approved inputs; the deterministic `generate` command below does not derive
   them during this demo.
...
For upstream context, see the Specadia project README.
