...
pre-reviewed, deterministic inputs; `specadia-contract generate` is the only
command that runs. Their possible provenance is described in step 3.

| # | Time | Command you type | Narration (spoken) | What appears on screen |
|---|------|------------------|---------------------|------------------------|
| 1 | 0:00 | `cd examples/bookmark-buddy` | "This is Bookmark Buddy, a tiny local-first command-line app for saving and searching bookmarks. It starts from one plain-English intent." | Terminal at the demo directory. |
...
| 3 | 0:22 | `cat requirements.md` | "That intent becomes reviewed requirements: authored by a person, produced with READ-MAS, or generated by Specadia with agents enabled. In this deterministic demo, the requirements are already approved inputs." | Purpose, FR-1..FR-3, NFR-1..NFR-2, AC-1 headings. |
| 4 | 0:44 | `cat design.md` | "A reviewed design adds the architecture and file structure, making implementation choices visible before coding begins." | Architecture and File Structure sections. |
